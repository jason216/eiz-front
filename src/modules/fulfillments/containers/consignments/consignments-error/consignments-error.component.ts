...
@Component({
  ...
  selector: 'consignments-error',
  templateUrl: './consignments-error.component.html',
  styleUrls: ['./consignments-error.component.scss'],
  providers: [
    ConsignmentsService,
  ],
})
export class ConsignmentsErrorComponent implements OnInit, OnDestroy {
  consignments;
      ...
      onGridReady: () => {
          ...
          this.consignmentsSubscription = this.activeContentService.onConsignmentsChange.subscribe(
            (consignments) => {
              ...
              this.gridOptions.api.setRowData(this.activeContentService.consignments.error);
            }
          );
